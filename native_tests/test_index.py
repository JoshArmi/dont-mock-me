...
def get_output(name: str) -> str:
    stdout = run(
        f"cd native_tests && terraform output {name}",
        shell=True,
        check=True,
        capture_output=True,
    ).stdout.decode()
...
def integration():
    if os.getenv("ENVIRONMENT") != "local":
        run("cd native_tests && terraform init", shell=True, check=True)
        run(
            "cd native_tests && terraform destroy -auto-approve",
            shell=True,
            check=True,
        )
        run("cd native_tests && terraform apply -auto-approve", shell=True, check=True)
